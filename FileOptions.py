...
def create_dataset_jbbm(cursor):
    ...
    all_info = DBOptions.getSQL(sql, cursor=cursor)
    dataset_jbbm = np.zeros((35308, 50, 1332), dtype='int32')
    ...
    all_info = DBOptions.getSQL(sql, cursor=cursor)
    label_jbbm = np.zeros((35308, 612), dtype='int16')
    print('诊断标签大小：', label_jbbm.shape)
    for i in all_info:
        label_jbbm[i[0], i[1]] = 1

    dataset_jbbm_train = dataset_jbbm[:30000]
    dataset_jbbm_test = dataset_jbbm[30000:]
    label_jbbm_train = label_jbbm[:30000]
    label_jbbm_test = label_jbbm[30000:]
    ...
    np.savez_compressed('./data_npz/label_jbbm_train.npz', label_jbbm_train)
    np.savez_compressed('./data_npz/label_jbbm_test.npz', label_jbbm_test)


# 创建诊断数据集
def create_dataset_duration(cursor):
    sql = 'select grbh_index,xh_index,duration from DATA_ANALYSIS_JBBM where XH_INDEX!=0'
    all_info = DBOptions.getSQL(sql, cursor=cursor)
    dataset_duration = np.zeros((35308, 50, 1), dtype='int32')
    print('诊断数据集大小：', dataset_duration.shape)
    for i in all_info:
        if i[1] < 50:
            dataset_duration[i[0], -i[1], 0] = i[2]

    sql = 'select grbh_index,duration from DATA_ANALYSIS_JBBM where XH_INDEX=0'
    all_info = DBOptions.getSQL(sql, cursor=cursor)
    label_duration = np.zeros((35308,1), dtype='int16')
    print('诊断标签大小：', label_duration.shape)
    for i in all_info:
        label_duration[i[0], 0] = i[1]

    dataset_duration_train = dataset_duration[:30000]
    dataset_duration_test = dataset_duration[30000:]
    label_duration_train = label_duration[:30000]
    label_duration_test = label_duration[30000:]
    np.savez_compressed('./data_npz/dataset_duration_train.npz', dataset_duration_train)
    np.savez_compressed('./data_npz/dataset_duration_test.npz', dataset_duration_test)
    np.savez_compressed('./data_npz/label_duration_train.npz', label_duration_train)
    np.savez_compressed('./data_npz/label_duration_test.npz', label_duration_test)
...
def create_dataset_drug(cursor):
    ...
    all_info = DBOptions.getSQL(sql, cursor=cursor)
    dataset_drug = np.zeros((28753, 50, 1125), dtype='float32')
    print('诊断数据集大小：', dataset_drug.shape)
    for i in all_info:
        if i[1] < 50:
    ...
    sql = 'select grbh_index,DRUG_CATEG_INDEX,ZJEITEM from DATA_ANALYSIS_DRUG,DATA_ANALYSIS_JBBM where ' \
          'DATA_ANALYSIS_JBBM.XH =DATA_ANALYSIS_DRUG.XH AND DATA_ANALYSIS_JBBM.XH_INDEX=0 '
    all_info = DBOptions.getSQL(sql, cursor=cursor)
    label_drug = np.zeros((35308, 136), dtype='float32')
    print('诊断标签大小：', label_drug.shape)
    for i in all_info:
        label_drug[i[0], i[1]] += i[2]

    dataset_drug_train = dataset_drug[:30000]
    dataset_drug_test = dataset_drug[30000:]
    label_drug_train = label_drug[:30000]
    label_drug_test = label_drug[30000:]
    np.savez_compressed('./data_npz/dataset_drug_train.npz', dataset_drug_train)
    np.savez_compressed('./data_npz/dataset_drug_test.npz', dataset_drug_test)
    np.savez_compressed('./data_npz/label_drug_train.npz', label_drug_train)
    np.savez_compressed('./data_npz/label_drug_test.npz', label_drug_test)
...
def create_dataset_drug_nocost(cursor):
    sql = 'select grbh_index,xh_index,DRUG_INDEX from DATA_ANALYSIS_DRUG,DATA_ANALYSIS_JBBM where ' \
          'DATA_ANALYSIS_JBBM.XH =DATA_ANALYSIS_DRUG.XH AND DATA_ANALYSIS_JBBM.XH_INDEX!=0'
    all_info = DBOptions.getSQL(sql, cursor=cursor)
    dataset_drug_nocost = np.zeros((28753, 50, 1096), dtype='float32')
    ...
    for i in range(12):
        sql = "select grbh_index,DRUG_CATEG_INDEX from DATA_ANALYSIS_DRUG,DATA_ANALYSIS_JBBM where " \
              "DATA_ANALYSIS_JBBM.XH =DATA_ANALYSIS_DRUG.XH AND DATA_ANALYSIS_JBBM.XH_INDEX=0 " \
              "and DATA_ANALYSIS_DRUG.ZYRQ < to_date('2014-10-29 00:00:00','yyyy-mm-dd hh24:mi:ss') "
        all_info_3month = DBOptions.getSQL(sql, cursor=cursor)
        label_drug_nocost_3 = np.zeros((28753, 135), dtype='float32')
        print('诊断标签大小：', label_drug_nocost_3.shape)
        for i in all_info_3month:
            label_drug_nocost_3[i[0], i[1]] =1

    dataset_drug_nocost_train = dataset_drug_nocost[:30000]
    dataset_drug_nocost_test = dataset_drug_nocost[30000:]
    label_drug_nocost_train = label_drug_nocost[:30000]
    label_drug_nocost_test = label_drug_nocost[30000:]
    ...
    np.savez_compressed('./data_npz/label_drug_nocost_train.npz', label_drug_nocost_train)
    np.savez_compressed('./data_npz/label_drug_nocost_test.npz', label_drug_nocost_test)
