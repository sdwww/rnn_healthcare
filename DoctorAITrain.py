...
def train_model(
    ...
    model.compile(optimizer='rmsprop',
                  ...
                  loss_weights={'jbbm_output': 1, 'sick_output': 1})
    ...
    model.fit(x=[dataset_jbbm_train, dataset_drug_train], y=[label_jbbm_train, label_sick_train],
              epochs=max_epochs, batch_size=batch_size, validation_split=0.2,callbacks=[history])
    model.save('./data_h5/rnn' + len(hidden_size) + '_emb'+emb_size+'_hidden' +hidden_size+'_'+ str(max_epochs) + 'epochs_' + str(month) + 'month.h5')
